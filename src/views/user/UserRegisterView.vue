<template>
  <div>
    <section class="login-section">
      <div class="wrapper">
        <nav><a href="javascript:;">注册</a></nav>
        ...
        <div class="form">
          <el-form ref="formRef" :model="form" :rules="rules" label-position="right" label-width="80px" status-icon>
            ...
            <el-form-item prop="userName" label="用户名&nbsp;&nbsp;">
              <el-input v-model="form.userName" clearable placeholder="请输入真实姓名" />
            </el-form-item>
            ...
            <el-form-item prop="checkPassword" label="确认密码">
              <el-input v-model="form.checkPassword" type="password" clearable show-password placeholder="请再次输入密码" />
            </el-form-item>
...
const rules = {
  ...
  checkPassword: [
    { required: true, message: '密码不能为空', trigger: 'blur' },
    // 添加自定义校验规则，确保密码与确认密码相同
    { validator: (rule:any, value:any, callback:any) => {
      if (value === form.userPassword) {
        callback();// 没有错误时调用 callback 并传入 null
      } else {
        callback(new Error('两次输入的密码不一致')); 
      }
    }, trigger: ['blur', 'change'] }, // 在失去焦点或值变化时触发验证
  ],
}
 ...
 */
 ...
 const handleSubmit = () => {
  ...
  formRef.value?.validate(async (valid:any) => {
    ...
    if (valid) {
      // TODO REGISTER
      await UserControllerService.userRegisterUsingPost(form)
      // 1. 提示用户
